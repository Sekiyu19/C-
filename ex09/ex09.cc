...
int main() {
    ...
    for(int i = 0; i < student_infos.size(); i++) {
        if (student_infos.at(i).exercise_scores().size()) {
            cout << fixed << setprecision(1) << "| " << left << setw(12) << student_infos.at(i).last_name() << " | " << setw(12) << student_infos.at(i).first_name() << " | " << setw(9) << student_infos.at(i).student_id() << " | " << right << setw(5) << student_infos.at(i).median() << " | " << setw(5) << student_infos.at(i).grade() << " |" << endl;
        } else {
            cout << fixed << setprecision(1) << "| " << left << setw(12) << student_infos.at(i).last_name() << " | " << setw(12) << student_infos.at(i).first_name() << " | " << setw(9) << student_infos.at(i).student_id() << " | No exercises  |" << endl;
        }
        count++;
        if(count % 5 == 0) {
            cout << "+--------------+--------------+-----------+-------+-------+" << count << endl;
